...
export default class GridView {
    ...
    enterModalDetails(divModalContent) {
        let html = `<div class="enterDetails"><input placeholder="title" autofocus class="title"/><input placeholder="year" class="year"/><input placeholder="director" class="director"/>
                    <input placeholder="actor" class="actor"/><input placeholder="imageUrl"  class="imageUrl"/><button class="addMovie">add movie</button><span class="addStatus"></span></div>`
        divModalContent.innerHTML = html
    ...
    addMovieModal() {
        ...
        divModalContent.className = 'modal-content'
        divMyModal.className = 'modal'
        this.enterModalDetails(divModalContent)
        // this.disableModalClick(divMyModal)
        divMyModal.appendChild(divModalContent)
    ...
    openAddMovieModal(movies) {
        this.addMovieModal()
        let lastId = movies[movies.length - 1].id
        let addMovieBtn = document.querySelector('.addMovie')
        addMovieBtn.onclick = () => {
            let newMovie = {
            ...
            api.addMovie(newMovie).then((data) => {//not update 
                console.log(data)
